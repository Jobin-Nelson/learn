...
def color_word(word: str, guess: str) -> list[tuple[int, int]]:
    ret = []
...
def c_main(stdscr: curses._CursesWindow) -> int:
    ...
    while True:
        ...
        if len(current_word) == 5:
            if current_word in set_wordlist:
                stdscr.addstr(line, 12, 'press enter to accept', curses.color_pair(CYAN_PAIR))
                ch = stdscr.get_wch()
                if ch == '\n':
                ...
                stdscr.addstr(line, 12, '(word not in word list!)', curses.color_pair(RED_PAIR))
        ...
        elif (len(current_word) < 5 and isinstance(ch, str) and ch.isascii() and ch.isalpha()):
