'''
Created Date: 2023-06-03
Qn: A company has n employees with a unique ID for each employee from 0 to n -
    1. The head of the company is the one with headID.

    Each employee has one direct manager given in the manager array where
    manager[i] is the direct manager of the i-th employee, manager[headID] =
    -1. Also, it is guaranteed that the subordination relationships have a tree
    structure.

    The head of the company wants to inform all the company employees of an
    urgent piece of news. He will inform his direct subordinates, and they will
    inform their subordinates, and so on until all employees know about the
    urgent news.

    The i-th employee needs informTime[i] minutes to inform all of his direct
    subordinates (i.e., After informTime[i] minutes, all his direct
    subordinates can start spreading the news).

    Return the number of minutes needed to inform all the employees about the
    urgent news.
Link: https://leetcode.com/problems/time-needed-to-inform-all-employees/
Notes:
    - use bfs
'''
from collections import deque

def numOfMinutes(n: int, headID: int, manager: list[int], informTime: list[int]) -> int:
    if len(informTime) == 1: return informTime[0]
    graph = [[] for _ in range(len(manager))]
    
    for i, m in enumerate(manager):
        if m == -1: continue
        graph[m].append(i)

    res = 0
    q: deque[tuple[int, int]] = deque([(headID, 0)])
    while q:
        id, time = q.popleft()
        res = max(res, time)
        for nei in graph[id]:
            q.append((nei, time + informTime[id]))
    return res

if __name__ == '__main__':
    n1, h1, m1, i1 = 1, 0, [-1], [0]
    n2, h2, m2, i2 = 6, 2, [2, 2, -1, 2, 2, 2], [0, 0, 1, 0, 0, 0]

    print(numOfMinutes(n1, h1, m1, i1))
    print(numOfMinutes(n2, h2, m2, i2))
